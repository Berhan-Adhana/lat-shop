...
export default async function ProductPage({ params }: Props) {
  ...
  const p = product as any; // access new fields safely
  ...
  const isBook = product.category?.slug === "books";
      ...
      <div className={s.productInner}>
        ...
        <div className={s.productLayout}>
          ...
          <div className={s.productInfo}>
            ...
            {/* Book: show author under title */}
            {isBook && p.author && (
              <p style={{ fontSize: 15, color: "#7a3f1d", fontStyle: "italic", marginBottom: 10, fontFamily: "system-ui" }}>
                by {p.author}
                {p.publisher && <span style={{ fontStyle: "normal", color: "#b86820" }}> · {p.publisher}</span>}
              </p>
            )}
            ...
            <div className={s.badgesRow}>
              {isBook ? (
                <>
                  {p.format   && <span className={`${s.badge} ${s.badgeHandmade}`}>📖 {p.format}</span>}
                  {p.language && <span className={`${s.badge} ${s.badgeOrigin}`}>🌐 {p.language}</span>}
                  {p.ageRange && <span className={`${s.badge} ${s.badgeSale}`}>👤 {p.ageRange}</span>}
                </>
              ) : (
                <>
                  {p.isHandmade !== false && <span className={`${s.badge} ${s.badgeHandmade}`}>🤲 Handcrafted</span>}
                  {p.origin && <span className={`${s.badge} ${s.badgeOrigin}`}>📍 {p.origin}</span>}
                  {product.onSale && <span className={`${s.badge} ${s.badgeSale}`}>🏷️ On Sale</span>}
                </>
              )}
            </div>
            ...
            <div className={s.priceBlock}>
              ...
              {product.onSale && p.salePrice && (
                <span className={s.originalPrice}>CA${p.price.toFixed(2)}</span>
              )}
              {product.onSale && p.salePrice && (
                <span className={s.savings}>Save CA${(p.price - p.salePrice).toFixed(2)}</span>
              )}
            </div>
            ...
            <div className={s.stockInfo}>
              {product.stock === 0 ? (
                ...
                <span className={s.lowStock}>🔥 Only {product.stock} left</span>
              ) : (
            ...
            {/* ── Product Tabs — now receives ALL fields ── */}
          ...
          <section style={{ marginTop: 64 }}>
            <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", marginBottom: 24 }}>
              <h2 style={{ fontSize: 22, fontFamily: "Georgia, serif", fontWeight: 400, color: "#1a1209" }}>
                {isBook ? "More Books" : "You May Also Like"}
              </h2>
              {product.category && (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 16 }}>
              {related.slice(0, 4).map((rel) => {
                const rp = rel as any;
                const relPrice = rel.onSale && rp.salePrice ? rp.salePrice : rel.price;
                const relIsBook = rel.category?.slug === "books";
                return (
                  <Link key={rel.id} href={`/product/${rel.slug}`} style={{ textDecoration: "none" }}>
                    <div style={{
                      background: "#fff", border: "1px solid #f4dbb0",
                      ...
                      onMouseEnter={(e) => {
                        ...
                        el.style.boxShadow = "0 8px 24px rgba(212,131,42,0.12)";
                      }}
                      onMouseLeave={(e) => {
                        const el = e.currentTarget as HTMLElement;
                        el.style.transform = "";
                    ...
                    >
                      <div style={{ aspectRatio: "1", background: "#faefd9", position: "relative" }}>
                        {rel.images?.[0] ? (
                          ...
                          <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 40, color: "#ecc07f" }}>
                            {relIsBook ? "📚" : "L"}
                          </div>
                        )}
                      </div>
                      <div style={{ padding: 12 }}>
                        <p style={{ fontSize: 13, fontWeight: 500, color: "#1a1209", fontFamily: "system-ui", marginBottom: 2 }}>
                          {rel.name}
                        </p>
                        {relIsBook && rp.author && (
                          <p style={{ fontSize: 11, color: "#b86820", fontFamily: "system-ui", fontStyle: "italic", marginBottom: 4 }}>
                            {rp.author}
                          </p>
                        )}
